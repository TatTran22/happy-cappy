...
1. **Startup with `avoid_text_cursor` already on** (returning user with feature enabled): `DesktopPetApp::init` calls `request_accessibility_on_startup_if_enabled(self.settings.avoid_text_cursor)` — passing the setting explicitly because the observer doesn't own `Settings`. The method prompts once and sets `prompted_for_accessibility_at_startup`. Idempotent within a session; no-op when the passed bool is false.
