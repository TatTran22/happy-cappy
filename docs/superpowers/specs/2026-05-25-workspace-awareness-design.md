...
- `src/app.rs` window-visibility composition (no new controller — `window_macos.rs` stays a collection of helper functions):
  ...
  - Tick cadence while auto-hidden: the tick loop continues to run so the workspace observer stays live, but throttled. `next_tick_interval` (app.rs:583-600) currently only checks `!self.pet_visible` (returns 5 s); the new precedence keeps the observer responsive enough for both fullscreen-exit detection AND AX-permission label freshness while Settings is open. Final precedence:
    1. `self.settings_window.as_ref().map_or(false, |w| w.is_visible())` → 500 ms (new, **highest priority**; while Settings is open the observer must poll fast enough to refresh the AX permission label, regardless of pet visibility). This branch fixes the otherwise-stale label case where the user hides the pet, opens Settings, and grants AX in System Settings — without this branch we'd be stuck at the 5 s cadence below.
    2. `!self.pet_visible` → 5 s (existing, user explicitly hid AND Settings is closed; pet state machine effectively paused, no observer-dependent UI to refresh).
...
- `src/settings_window_macos.rs` (extended):
  - **Panel resize is required.** Today the panel is `PANEL_WIDTH=420.0` × `PANEL_HEIGHT=370.0` (lines 44-45), with hard-coded y positions from title at 322 down to the bottom button row at 28. There is no room for a section heading + 3 checkboxes + multi-line label + button without overlap. The spec requires:
    ...
    - The new block, top-down within the new range, with `MARGIN_X` matching existing usage:
      ...
      - "Avoid text-cursor area" checkbox at `y=128`, height 22.
      - "Auto-hide when any app is fullscreen" checkbox at `y=98`, height 22.
      - AX status label (`NSTextField`, non-editable, `setLineBreakMode:NSLineBreakByWordWrapping`, `setMaximumNumberOfLines:2`) at `y=58`, height 36, full row width.
      - "Re-request Accessibility permission" `NSButton` at `y=22`, height 28, width 280.
    - Each new control wired to `CommandTarget` exactly like existing controls (checkboxes → `settings_value_selector()`, button → `command_selector()`). Each carries the appropriate `MENU_TAG_*`.

    Plan stage is free to adjust the exact y values; the spec commits to *resizing* the panel and *not* overlapping with the existing controls.
...
- `AppCommand::SetAvoidTextCursor(true)` checks AX trust state first via `AXIsProcessTrusted()` (a non-prompting query). If trusted, no further action. If NOT trusted, calls `request_accessibility_now()` — which always invokes `AXIsProcessTrustedWithOptions` with the prompt option. We do NOT try to programmatically distinguish "dialog is showing right now" from "macOS suppressed the dialog after sticky denial", because both states return the same `AXIsProcessTrusted() == false` until the user acts.

  **Checkbox state vs degraded behavior** — explicit policy: the checkbox stays checked (reflecting the user's *intent* — the setting really is `true`), but the inline `ax_status_label` immediately under it is set to a neutral guidance string whenever `settings.avoid_text_cursor && !AXIsProcessTrusted()` — wording like *"Permission needed. If no dialog appears, click Re-request or open System Settings → Privacy & Security → Accessibility."* The label clears once `AXIsProcessTrusted()` returns true on a subsequent poll (refreshed via the observer trust-transition path). The pet's behavior degrades silently — `caret_rect = None` means the `AvoidRectHorizontal` arm never fires — but the UI is honest about *why* (checkbox checked + visible permission-needed label) rather than presenting either a silently-failing checkbox OR an automatically-reverted toggle that fights the user.

  Rejected alternative: reverting the checkbox to unchecked when permission is missing. That would (a) fight the user's expressed intent, (b) lose state across permission grants (we'd need to remember "user wanted this but couldn't have it"), and (c) make the Re-request button useless because the toggle would already be off.
...
- Whether to add a brief "fading" animation before window hide rather than instant hide. Punted unless it looks bad in manual testing.
- Whether the pre-existing `scale_factor` source at `app.rs:234-243` (uses `window.scale_factor()` even for non-current displays) should be fixed in the same plan. The workspace-awareness spec is correct by construction either way because `bounds_logical` tracks `self.physics.bounds`, but the latent bug is worth raising.
